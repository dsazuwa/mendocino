...
describe('set cookie schema', () => {
  it('should pass for valid data', () => {
    const data = {
      body: { accessToken: '437ewskjc', refreshToken: '437ewskjc' },
    };

    expect(() => setCookieSchema.parse(data)).not.toThrow();
  });

  it('should fail for valid data', () => {
    const data = {
      body: { accessToken: '', refreshToken: '' },
    };

    expect(() => setCookieSchema.parse(data)).toThrow();
  });
});
