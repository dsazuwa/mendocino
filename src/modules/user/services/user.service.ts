...
const getUserWithoutId = async (userId: number, isAdmin: boolean) => {
...
const getUserForSocialAuthentication = async (
  identityId: string,
...
const getUserForRecovery = async (email: string) => {
  const query = `
    SELECT
      is_admin AS "isAdmin",
...
const userService = {
  ...
  getUserByEmail,
  getUserFromPayload,
  getUserForSocialAuthentication,
  getUserForRecovery,
};
